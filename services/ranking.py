...
class RankingService:
    def __init__(self, logger: logging.Logger, helper_repository: HelperRepository):
        self.logger = logger
    ...
    def generate(self):
        ...
        for rank in self.repository.get_whole_rank():
            ...
            if rank.points <= 0:
                continue
            data += (
                f'{":first_place:" if place == 1 else ":second_place:" if place == 2 else ":third_place:" if place == 3 else "`"+str(place)+".`"} '
