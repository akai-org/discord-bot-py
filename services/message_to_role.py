...
class MessageToRoleService:
    ...
    async def add_role(self, message_id, user: discord.Member):
        ...
        if role in user.roles:
            self.logger.warning(
                f"User {user.display_name} added reaction to a role message with ID "
        ...
        self.logger.info(
            f"User {user.display_name} receives role {role.name}" f" in {user.guild.name} due to their reaction"
        )
    ...
    async def remove_role(self, message_id, user: discord.Member):
        ...
        if role not in user.roles:
            self.logger.warning(
                f"User {user.display_name} removed reaction to a role message with ID "
                f"{message_id}, but their role {role.name} did not change for "
        ...
        self.logger.info(
            f"User {user.display_name} loses role {role.name}" f" in {user.guild.name} due to removal of their reaction"
        )
