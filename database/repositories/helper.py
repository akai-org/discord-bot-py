...
class HelperRepository(Repository):
    def __init__(
        self, sessionmaker: session, user_rank_model: Base, helper_range_model: Base, helper_reward_model: Base
    ):
    ...
    def add_points_to_user(self, user_id, points):
        ...
        user_points = (
            session.execute(select(self.user_rank_model.points).where(self.user_rank_model.user_id == user_id))
            .scalars()
            .first()
        )
        if user_points is None:
            ...
            session.execute(
                update(self.user_rank_model)
                .where(self.user_rank_model.user_id == user_id)
                .values(points=user_points + points)
            )
        session.commit()
        session.close()
    ...
    def role_id_user_should_have(self, user_id) -> int:
        ...
        role_id = (
            session.execute(
                select(self.range_model.role_id)
                .order_by(self.range_model.bottom_threshold.desc())
                .where(self.range_model.bottom_threshold <= user_points)
            )
            .scalars()
            .first()
        )
    ...
    def get_whole_rank(self) -> list:
        session = self.sessionmaker()
        users = (
            session.execute(select(self.user_rank_model).order_by(self.user_rank_model.points.desc())).scalars().all()
        )
        session.close()
        return users
    ...
    def get_reward(self, emoji_name) -> int:
        session = self.sessionmaker()
        reward = (
            session.execute(select(self.reward_model.reward).where(self.reward_model.emoji_name == emoji_name))
            .scalars()
            .first()
        )
        session.close()
